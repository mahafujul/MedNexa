...
// Define the PUT endpoint to reschedule an appointment
export default async function PUT(request: NextRequest) {
    try {
        // Retrieve data from request body
        const reqBody = await request.json();
        const { date, selectedTimeSlot } = reqBody;
        ...
        // Find the appointment by its ID and update its date and selectedTimeSlot
        const appointmentAfterUpdate = await Appointment.findByIdAndUpdate(appointmentId, { date, selectedTimeSlot });
        ...
        // Return success response indicating the appointment has been successfully rescheduled
        return NextResponse.json({ message: "Appointment successfully rescheduled.", success: true }, { status: 200 });
    } catch (err: any) {
